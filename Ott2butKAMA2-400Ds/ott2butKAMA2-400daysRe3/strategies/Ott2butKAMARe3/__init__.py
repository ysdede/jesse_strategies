...
class Ott2butKAMARe3(Strategy):
    ...
    def hyperparameters(self):
        return [
            {'name': 'ott_ma_len', 'type': int, 'min': 5, 'max': 105, 'default': 65},
            {'name': 'ott_percent', 'type': int, 'min': 1500, 'max': 6500, 'default': 1940},
            {'name': 'stop_loss', 'type': int, 'min': 1000, 'max': 3000, 'default': 1990},
            {'name': 'risk_reward', 'type': int, 'min': 200, 'max': 1200, 'default': 880},
    ...
    def update_position(self):
        # print('\n', self.candles[-1, 0], type(self.candles[-1, 0]))
        if self.is_long:
            if self.cross_down:
                self.liquidate()
        if self.is_short:
