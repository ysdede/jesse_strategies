...
class Ott2butKAMA(Strategy):
    ...
    def hyperparameters(self):
        return [
            {'name': 'ott_len', 'type': int, 'min': 1, 'max': 50, 'default': 36},  # 12
            ...
            {'name': 'chop_rsi_len', 'type': int, 'min': 2, 'max': 50, 'default': 17},
            {'name': 'chop_bandwidth', 'type': int, 'min': 10, 'max': 300, 'default': 144},
        ]
    ...
    @cached
    def pos_size(self):
        return (utils.size_to_qty((self.capital / 20), self.price, fee_rate=self.fee_rate) * self.leverage) + 0.001
