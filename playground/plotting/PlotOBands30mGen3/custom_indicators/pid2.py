...
def pid2(candles: np.ndarray,
    ...
    pid1.output_limits = (10000, 50000)
    ...
    for i in range(1, source.size-1):
        ...
        out[i] = pid1(diff)
        # out[i] = pid1(source[i])
        forecast[i + 1] = out[i]  # * (1 + (out[i] / 1000))
        print(
            f'forecast[i] {forecast[i]} source[i] {source[i]} diff: {diff} out[i] {out[i]} forecast[i] {forecast[i]}')
