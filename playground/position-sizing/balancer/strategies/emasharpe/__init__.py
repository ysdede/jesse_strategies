...
class emasharpe(Strategy):
    ...
    def __init__(self):
        ...
        self.header1 = ['Time', 'AAVE', 'ETH', 'BNB', 'BTC', 'NEO', 'QTUM', 'Mean',
                        'AAVE dev', 'ETH dev', 'BNB dev', 'BTC dev', 'NEO dev', 'QTUM dev',
                        'AAVE ndev', 'ETH ndev', 'BNB ndev', 'BTC ndev', 'NEO ndev', 'QTUM ndev',
                        'Sum Norm. Dev.']
        # self.csv_rows = np.array(self.header1)
        self.results = []
        self.formatter = '{: <12} {: <6} {: <6} {: <6} {: <6} {: <6} {: <6} {: <6} {: <8} {: <8} {: <8} {: <8} {: <8} {: <8} {: <9} {: <9} {: <9} {: <9} {: <9} {: <9} {: <9} '
    ...
    def stats(self):
        ...
        if hour == '00' and minute == '00':
            ...
            if self.symbol.startswith('ETH-'):
                ...
                sharpes = [self.shared_vars['AAVE-USDT']['Sharpe'], self.shared_vars['ETH-USDT']['Sharpe'],
                           self.shared_vars['BNB-USDT']['Sharpe'], self.shared_vars['BTC-USDT']['Sharpe'],
                           self.shared_vars['NEO-USDT']['Sharpe'], self.shared_vars['QTUM-USDT']['Sharpe']]
                mean = round(statistics.mean(sharpes), 3)

                sharpes = [round(x, 2) for x in sharpes]
                ...
                self.shared_vars['AAVE-USDT']['MyShare'] = self.normalized_deviations[0]
                self.shared_vars['ETH-USDT']['MyShare'] = self.normalized_deviations[1]
                self.shared_vars['BNB-USDT']['MyShare'] = self.normalized_deviations[2]
                self.shared_vars['BTC-USDT']['MyShare'] = self.normalized_deviations[3]
                self.shared_vars['NEO-USDT']['MyShare'] = self.normalized_deviations[4]
                self.shared_vars['QTUM-USDT']['MyShare'] = self.normalized_deviations[5]
                ...
                if self.days == period:
                    self.createreport(self.results, 'sharpies.csv')
